...
def get_business_seconds(request):
    ...
    try:
        ...
        return PyramidResponse(
            json={'error': 'Start time and End time should be in isoformat'},
            status=400,
        )
    except KeyError:
        return PyramidResponse(
            json={'error': 'Invalid key found in path params'}, status=400
        )
    response = request.registry.business_controller.business_seconds(
        start, end
    )
    print(f'Response from controller = {response} and type: {type(response)}')
    return PyramidResponse(
        json={'business_seconds': int(response) if response else int('0')},
        status=200,
    )
